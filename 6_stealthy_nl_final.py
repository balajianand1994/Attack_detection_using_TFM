...
Nline   = len(branches)
n_state = (Nbus - 1) + Nbus   # 27: 13 angles + 14 frequencies
m_meas  = Nbus + Nline         # 34: 14 frequencies + 20 line flows
dt      = 0.02

# Synthetic line susceptances
Bline = 4.0 * np.ones(Nline)
# Bline[(branches[:,0]==0) | (branches[:,1]==0)] = 5.0
# Bline[(branches[:,0]==3) | (branches[:,1]==3)] = 4.5
# Bline[(branches[:,0]>=8) | (branches[:,1]>=8)] = 3.0
...
# M_bus = np.ones(Nbus)
# D_bus = 0.02 * np.ones(Nbus)


M_bus = [1.06,1.045,1.01,1.019,1.02,1.07,1.062,1.09,1.056,1.051,1.057,1.055,1.05,1.036]

#  Small damping gives weakly damped / near-marginal oscillatory behavior.
...
Q_noise = 1e-5 * np.eye(n_state)   # small process noise for EKF
...
# ============================================================
def f_swing(x):
    """Nonlinear swing equation — no input."""
    theta = x[:Nbus-1]
    omega = x[Nbus-1:]
...
def h_meas(x):
    """Nonlinear measurement: bus frequencies + line flows."""
    theta = x[:Nbus-1]
    omega = x[Nbus-1:]
    delta = np.concatenate([[0.0], theta])
    flows = np.array([
...
context_len     = 100
alpha           = 0.001
buffer_weight   = 1      # weight on TimesFM prediction when alarm fires

T_warmup        = context_len + 2
T_clean         = 1
attack_duration = 50
...
eta_scale       = 0.1
eta             = eta_scale * threshold   # attack energy budget
threshold       = 1.05*threshold

# bias direction: frequency at bus 2
q               = np.zeros(n_state)
q[Nbus - 1 + 1] = 1.0
q              /= np.linalg.norm(q)

assert context_len <= T_warmup
print(f"n={n_state} | m={m_meas} | threshold={threshold:.3f} | eta={eta:.4f}")


# ============================================================
# ESTIMATE SIGMA_FM ONCE — from a dedicated clean run
# ============================================================
np.random.seed(5)

theta0_s   = 0.08 * np.random.randn(Nbus - 1)
omega0_s   = 0.02 * np.random.randn(Nbus)
x_sigma    = np.zeros((T_trial + 1, n_state))
x_sigma[0] = np.concatenate([theta0_s, omega0_s])
y_sigma    = np.zeros((T_trial, m_meas))

for t in range(T_trial):
    x_sigma[t+1] = f_swing(x_sigma[t])
    y_sigma[t]   = h_meas(x_sigma[t]) + Rhalf @ np.random.randn(m_meas)

res_fm_init = np.zeros((T_clean, m_meas))
for t in range(T_clean):
    global_t = T_warmup + t
    start    = max(0, global_t - context_len)
    contexts = [y_sigma[start:global_t, ch] for ch in range(m_meas)]
    pf, _    = model.forecast(horizon=1, inputs=contexts)
    res_fm_init[t] = y_sigma[global_t] - pf[:, 0]

Sigma_fm     = np.cov(res_fm_init.T)
Sigma_fm_inv = np.linalg.inv(Sigma_fm)
print(f"Sigma_fm estimated once | diag mean: {np.diag(Sigma_fm).mean():.4f}")

# ============================================================
# MONTE CARLO Version — proper EKF relinearization + TimesFM
# ============================================================
...
for trial in range(N_trials):
    ...
    ypred_nom_t = np.zeros((T_trial, m_meas))
    K_store_t   = np.zeros((T_trial, n_state, m_meas))
    S_store_t   = np.zeros((T_trial, m_meas, m_meas))
    F_store_t   = np.zeros((T_trial, n_state, n_state))

    # --------------------------------------------------------
    # Step B: nominal EKF — store gains for attack design only
    # --------------------------------------------------------
    for t in range(T_trial):
        ...
        P_t          = (np.eye(n_state) - Kk @ Hk) @ Ppred @ \
    ...
    # --------------------------------------------------------
    # Step C: backward pass — attack design (nominal linearization)
    # --------------------------------------------------------
    lam_t       = q.copy()
    delta_att_t = np.zeros((T_trial, m_meas))

    for t in range(T_trial - 1, -1, -1):
        if t >= attack_start:
            Kk    = K_store_t[t]
            Sk    = S_store_t[t]
            ck    = Kk.T @ lam_t
            denom = np.sqrt(ck @ Sk @ ck)
            if denom > 1e-12:
                delta_att_t[t] = np.sqrt(eta) * Sk @ ck / denom
        lam_t = F_store_t[t].T @ lam_t

    print(f"    Attack design done.")

    # --------------------------------------------------------
    # Step D: proper attacked + clean EKF (relinearize own estimate)
    # --------------------------------------------------------
    ...
    P_att = 0.01 * np.eye(n_state)
    P_cln = 0.01 * np.eye(n_state)

    y_att        = y_cln.copy()
    res_ekf_att  = np.zeros((T_online_trial, m_meas))
    res_ekf_cln  = np.zeros((T_online_trial, m_meas))
    S_att_store  = np.zeros((T_online_trial, m_meas, m_meas))
    S_cln_store  = np.zeros((T_online_trial, m_meas, m_meas))

    for t in range(T_trial):

        # --- proper attacked EKF ---
        xpred_att  = f_swing(xhat_att[t])
        Fk_att     = numerical_jacobian(f_swing, xhat_att[t])
        Ppred_att  = Fk_att @ P_att @ Fk_att.T + Q_noise
        ...
        if t >= attack_start:
            a_t      = ypred_att - ypred_nom_t[t] + delta_att_t[t]
            y_att[t] = y_cln[t] + a_t

        r_att_t       = y_att[t] - ypred_att
        xhat_att[t+1] = xpred_att + Kk_att @ r_att_t
        P_att         = (np.eye(n_state) - Kk_att @ Hk_att) @ Ppred_att @ \
                        (np.eye(n_state) - Kk_att @ Hk_att).T + Kk_att @ R_noise @ Kk_att.T
        P_att         = 0.5 * (P_att + P_att.T) + 1e-12 * np.eye(n_state)

        # --- proper clean EKF ---
        xpred_cln  = f_swing(xhat_cln[t])
        Fk_cln     = numerical_jacobian(f_swing, xhat_cln[t])
        Ppred_cln  = Fk_cln @ P_cln @ Fk_cln.T + Q_noise
        ...
        r_cln_t       = y_cln[t] - ypred_cln
        xhat_cln[t+1] = xpred_cln + Kk_cln @ r_cln_t
        P_cln         = (np.eye(n_state) - Kk_cln @ Hk_cln) @ Ppred_cln @ \
                        (np.eye(n_state) - Kk_cln @ Hk_cln).T + Kk_cln @ R_noise @ Kk_cln.T
        P_cln         = 0.5 * (P_cln + P_cln.T) + 1e-12 * np.eye(n_state)

        if t >= T_warmup:
            online_t              = t - T_warmup
            res_ekf_att[online_t] = r_att_t
            res_ekf_cln[online_t] = r_cln_t
            S_att_store[online_t] = Sk_att
            S_cln_store[online_t] = Sk_cln

    print(f"    Proper EKF done.")

    # --------------------------------------------------------
    # Step E: TimesFM residuals (on new y_att from proper EKF)
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    chi2_ekf_att = np.array([res_ekf_att[t] @ np.linalg.solve(S_att_store[t], res_ekf_att[t])
                              for t in range(T_online_trial)])
    chi2_ekf_cln = np.array([res_ekf_cln[t] @ np.linalg.solve(S_cln_store[t], res_ekf_cln[t])
                              for t in range(T_online_trial)])
...
# ============================================================
# PLOT  (same structure as replay_attack.py)
# ============================================================
fig = plt.figure(figsize=(14, 10.5))
gs  = gridspec.GridSpec(3, 2, figure=fig)

# --- Row 0: frequency at bus 2 ---
ax0        = fig.add_subplot(gs[0, :])
omega2_idx = 1   # omega_2 is index 1 in measurement vector
...
ax0.set_title('IEEE 14-bus — optimal stealthy attack against EKF')
...
plot_band(ax_fm_att, chi2_fm_att_trials, color='steelblue',
          title=f'TimesFM — optimal attack (duration={attack_duration})',
...
ax_fm_att.set_ylim(0, y_max_att);  ax_ekf_att.set_ylim(0, y_max_cln)

plt.suptitle(
    f'IEEE 14-bus Optimal Attack | N_trials={N_trials} | context_len={context_len} | $\\alpha$={alpha}',
    fontsize=13, fontweight='bold'
)
plt.tight_layout()
plt.savefig('ieee14_ekf_optimal_attack.pdf', bbox_inches='tight')
plt.show()
